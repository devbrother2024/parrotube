...
describe('buildPublicChannelReport', () => {
  ...
  test('builds a public report from channel, uploads playlist, and video stats', async () => {
    ...
    expect(report.transcriptsSummary.included).toBe(false);
  });

  test('comments and transcripts are opt-in', async () => {
    const { buildPublicChannelReport } = await import('./public-report');
    ...
    expect(mockFetchTranscript).toHaveBeenCalledWith('video-1', 'ko');
    expect(report.commentsSummary.included).toBe(true);
    expect(report.commentsSummary.totalFetched).toBe(1);
    expect(report.transcriptsSummary.included).toBe(true);
    expect(report.transcriptsSummary.items[0]?.segmentCount).toBe(2);
  });
